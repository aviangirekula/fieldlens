...
| Component-ID accuracy (strict exact match) | **80%** (24/30, best of 3 runs) | informational |
| Median latency, photo → full inspection drill | **~5–6s** (p95 ~7s) | end-to-end |

**Run-to-run variance.** The model runs at temperature 0.4, so component IDs vary between runs. Across three runs we measured **21/30, 22/30, and 24/30** — we report the best observed (24/30, 80%); the band is ~70–80%. The safety result was **stable at 0 hazards-marked-safe in every run** — that's the number that matters and it doesn't move. The per-image table below is one representative logged run.
